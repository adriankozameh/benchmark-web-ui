...
The dashboard calls `GET /api/v1/organizations/{organizationId}/stations/{stationId}/timeseries/forecast` with a Cognito access token. Pick a station and a 24-hour, 3-day, 7-day, or 16-day window; use the provider chips to compare or hide individual providers. Chart labels show the station's local time zone from the forecast response (falling back to the station's `timeZone` for older APIs); hover labels include the UTC offset so repeated hours during daylight saving changes are distinct. Forecast points stay positioned by `utcDateTime` so providers align at the same instant. The charts show only data returned for the selected station; missing provider points are not filled in. An empty state appears when no forecast is available.
